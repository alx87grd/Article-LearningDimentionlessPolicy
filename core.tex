...
\caption{For the pendulum swing-up task, the context includes 5 variables: the parameter of the system: $m$, $g$, and $l$, a parameter of the cost function: $q$ and a parameter defining the constraints: $\tau_{max}$. The control input solution is a function of all those variables in addition of the current states.
...
\end{align}
%%%%%%%%%%%%%%%%%%%%%%
where exposant $e$ are rational numbers selected to make all equations dimentionless. Then, the Buckingham $\pi$ theorem tell us that the relationship described by eq. \eqref{eq:scalarpolicy} can be restated as the following relationship between  dimentionsless variables:
%%%%%%%%%%%%%%%%%%%%%%
\begin{equation}
u_j^* = \pi_j^* \left(
x_1^*, \hdots, x_n^*, 
...
\end{equation}
%%%%%%%%%%%%%%%%%%%%%%
involving $d$ less dimentionless variables. If we apply the same procedure to all control inputs, when can then assemble the $k$ mapping back into a vector form:
%%%%%%%%%%%%%%%%%%%%%%
\begin{equation}
\underbrace{
\begin{bmatrix}
...
\end{equation}
%%%%%%%%%%%%%%%%%%%%%%
Note that here, the cost function parameter $q$ is included in way to have units of torque, it was chosen to set to zero the weight on velocity for simplicity, and the weight on torque to one without loss of generality as only the relative values of weights will impact the solution. 
...
In some situation, changing a context variable will have little effect on the optimal policy. For instance, for the torque-limited optimal pendulum swing-up problem, augmenting $\tau_{max}$ or $q$ while keeping the other value fixed will have little effect pass a given threeshold. For instance, if we look at the solutions for context no 4, 5 and 6, using a lot of torque is so highly penalize by the cost function that the saturation limit is not really impacting the solution (except edges cases on the boundary), hence we would expect that augmenting $\tau_{max}$ should not change the solution. 

A sensivity analysis was conducted ...
...
\left\{
\begin{array}{c}
\pi^*_{b}( \theta^* , \dot{\theta}^* , q^* ) \; if \;  R^* \gg 1 \\
\pi^*_{u}( \theta^* , \dot{\theta}^* , \tau_{max}^* ) \; if \; R^* \ll 1
\end{array}
\right. \\
\text{with} \quad &R^* = \frac{\tau^*_{max}}{q^*} = \frac{\tau_{max}}{q} 
\end{align}
%%%%%%%%%%%%%%%%%%%%%%
...
Trajectory solutions are also generalizable in their dimentionless forms. Figures \ref{fig:c1} to \ref{fig:c9} show optimal trajectory solutions starting from the bottom position at rest. Those trajectories were computed  by executing the computed optimal policies in a simulation. 

Instead of thinking about computing a state feedback law, using the tool as a trajectory generator, we must think about 3 policies defining time-trajectory for states and inputs:
%%%%%%%%%%%%%%%%%%%%%%
\begin{align}
\tau          = \pi_{\tau}( t , m , g , l , q , \tau_{max} ) \\
\theta        = \pi_{\theta}( t , m , g , l , q , \tau_{max} ) \\
\dot{\theta } = \pi_{\dot{\theta }}( t , m , g , l , q , \tau_{max} )
\end{align}
%%%%%%%%%%%%%%%%%%%%%%

but time must also be scaled:
%%%%%%%%%%%%%%%%%%%%%%
\begin{align}
