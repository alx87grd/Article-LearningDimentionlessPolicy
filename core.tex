...
To consider the question of how can this policy or knowledge can be transfer in a different context, it is usefull to think about a higher dimension mapping, that we will note $\pi$, also having input arguments a vector of variables $c$ describing the context:
%%%%%%%%%%%%%%%%%%%%%%
\begin{equation}
u
=
...
It is usefull to define transformation matrices based on scalar equations \eqref{eq:piu}, \eqref{eq:pix} and \eqref{eq:pic}:
%%%%%%%%%%%%%%%%%%%%%%
\begin{align}
...
\end{align}
%%%%%%%%%%%%%%%%%%%%%%
can be found by scaling the input and output of $f_a$ using matrices $T_u$ and $T_x$:
%%%%%%%%%%%%%%%%%%%%%%
\begin{align}
f_a^* ( x^* ) &= T_u(c_a) 
\underbrace{
...
\end{align}
%%%%%%%%%%%%%%%%%%%%%%
and the dimentionless context values can be found using $T_c$:
%%%%%%%%%%%%%%%%%%%%%%
\begin{align}
c_a^* &= T_c( c_a ) \; c_a 
\end{align}
%%%%%%%%%%%%%%%%%%%%%%
Inversely, if we know a dimentionless feedback law, matrices $T_u$ and $T_x$ can be used to scale it to a specific context $c$:
%%%%%%%%%%%%%%%%%%%%%%
\begin{align}
f ( x ) &= T^{-1}_u(c) 
\underbrace{
...
%%%%%%%%%%%%%%%%%%%%%%
 \begin{figure}[ht]
    \centering
    \vspace{-10pt}
    \subfloat[Dimentional \label{fig:b}]{\includegraphics[width=0.20\textwidth]{fig/policy.jpg}}   
    \subfloat[Dimentionless \label{fig:c}]{\includegraphics[width=0.30\textwidth]{fig/dimpolicy.jpg}}
    \caption{subfigures}
...
One interesting perk of this dimentional analysis, is that we can remove $p$ variable form the context (typically $p$ would be 2 or 3 for controlling a physical system involving time, force and length). The global problem of learning $\pi(x,c)$, i.e. the good feeback policy for all possible context is thus simplified in a dimentionless form. But more importantly, it shows how different contexts will have the same dimentionless feedback law. In other words, the knowledge of a policy for a specific context $c$ can actually be generalized to a sub-space of all context for which the dimentionless context $c^*$ is equal. For instance, if a numerical algorithm find the optimal policy $f_a$ for specific context defined by variables $c_a$. Then we can scale the input and output to obtain the equivalent dimentionless relationship:
%%%%%%%%%%%%%%%%%%%%%%
\begin{equation}
f_1^*(x^*) = \pi^*( x^* , c^* = c_1^*)
\end{equation}
...
\end{equation}
%%%%%%%%%%%%%%%%%%%%%%
This is simply stating that the dimentionless policy (eq. \eqref{eq:vectordimpolicyshort}) give the same output for the same inputs. Finally, in order to use $f_2^*$ we would only have to rescale, using the context no2 parameters, to retrieve the dimentional version $f_2$. Applying $f_2$ policy on the system and task described by the context $c_2$ should be exactly equivalent as applying policy $f_1$ on the system and task described by the context $c_1$. 
