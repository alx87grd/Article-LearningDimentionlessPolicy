...
Using an analogy from fluid mechanics, when the Mach number is small $Ma < 0.3$, we can generaly assume to be in an incompressible regime where various speed of sound parameters (for instance having $Ma=0.1$ or $Ma=0.3$) would not really change the behavior.
...
Trajectory solutions are also generalizable in their dimentionless forms. Figures \ref{fig:c1} to \ref{fig:c9} show optimal trajectory solutions starting from the bottom position at rest. Those trajectories were computed  by executing the computed optimal policies in a simulation. We can also see that withing the 3 dimentionaly similar sub-groups, the optmial trajectories are actually the same but scalled.

Instead of thinking about computing a state feedback law, using the tool as a trajectory generator, we must think about computing 3 time-based open-loop feedback law defining trajectory for states and inputs:
%%%%%%%%%%%%%%%%%%%%%%
\begin{align}
\underbrace{
\tau          = \pi_{\tau}( t 
